...
```
sdk/                          ← Shared interfaces (future modules implement these)
  IProtectionModule.h         ← Abstract interface: Initialize/Connect/Disconnect/Shutdown
kernel/                       ← Kernel driver source
  RansomShield.c              ← DriverEntry, filter registration, I/O callbacks
  RansomShield.h              ← Kernel types, constants, function declarations
  RsContext.c                 ← Per-PID hash table, heuristic engine, allowlist
  RsCommPort.c                ← FltMgr comm port setup and message dispatch
usermode/
  common/                     ← Shared user-mode code (used by both CLI and tray)
    SharedDefs.h              ← Kernel↔user protocol structures (included by both sides)
    CommManager.h/.cpp        ← Parameterized; wraps FilterConnect/Send/GetMessage + listener thread
    ConfigManager.h/.cpp      ← Parameterized; registry persistence under configurable base path
    EventLogger.h/.cpp        ← Parameterized; Windows Event Log + file log
    RansomShieldModule.h/.cpp ← Owns the three managers; implements IProtectionModule
  cli/                        ← CLI application
    Main.cpp                  ← Argument dispatch, daemon loop, uses RansomShieldModule
  tray/                       ← Tray background agent
    TrayMain.cpp              ← WinMain, hidden message window, 5 s reconnect timer
    NotificationManager.h/.cpp← Shell_NotifyIcon, balloon tips, alert queue, context menu
```

## Architecture

This is a three-component Windows security system:

1. **Kernel driver** (`RansomShield.sys`) — WDM minifilter that blocks ransomware
2. **User-mode CLI** (`RansomShieldClient.exe`) — C++ CLI + daemon for monitoring and control
3. **Tray agent** (`RansomShieldTray.exe`) — GUI background agent with balloon notifications

### Module pattern

Both executables instantiate a `RansomShieldModule` (implements `IProtectionModule`) which owns the three parameterized managers. Adding a future protection engine (e.g. network filter) means:
1. Implement `IProtectionModule` in a new module class.
2. Create parameterized `CommManager`/`ConfigManager`/`EventLogger` with module-specific port/registry/source names.
3. Add the module to the host process alongside `RansomShieldModule`.
...
**Heuristic engine** (`kernel/RsContext.c`):
...
**Communication port** (`kernel/RsCommPort.c`):
...
### User-Mode Managers (`usermode/common/`)

| File | Responsibility |
|---|---|
| `SharedDefs.h` | All protocol structures shared between kernel and user mode |
| `CommManager` | Takes port name in constructor; wraps `FilterConnect/Send/GetMessage`; listener thread |
| `ConfigManager` | Takes registry base path in constructor; no longer calls CommManager directly |
| `EventLogger` | Takes event source name in constructor |
| `RansomShieldModule` | Composes the three managers; implements `IProtectionModule`; exposes `PushToDriver()`, `StartListener()` |

### Tray Agent (`usermode/tray/`)

Windows-subsystem background application (no console window). Uses `RansomShieldModule` for all driver interaction.
...
- Auto-start uses a Task Scheduler task (`RansomShieldTray`) with `RunLevel = HighestAvailable` so the tray starts elevated at logon without a UAC prompt. The old `HKCU\Run` entry is removed on first launch.

### Protocol (`usermode/common/SharedDefs.h`)
